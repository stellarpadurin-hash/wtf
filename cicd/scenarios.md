...
Here is exactly how you should structure and configure each trigger option.
------------------------------
...
* How it works: You deploy a Kubernetes Tekton CronTrigger / Trigger CRD or an external cron utility to fire a request every night at midnight.
...
If you are setting this up for the first time, start with Scenario 1 (The PR Trigger). It provides immediate value by catching flaws before they are merged and teaches your development team how the automated remediation sandbox behaves. Once that workflow is stable, implement Scenario 3 (The Scheduled Nightly Scan) to clean up older, inactive codebases automatically.
Which of these three tracking scenarios fits best with your team's current development workflow, or would you like to see how to structure the Tekton CronJob YAML for the scheduled nightly scan?
