# Post Actions When PR raised for breaking security fix
...
* **Locks the Original PR**: This failure sends a FAILED status back to Bitbucket Data Center [1]. Because of your branch permissions, the developer's original PR cannot be merged with the vulnerable dependencies intact
...
To prevent the automation from getting stuck in an infinite loop, the Tekton EventListener must use an interceptor filter. It intercepts the webhook from this new bot PR and treats it differently than a human PR.
...
| Bitbucket Status | Marks PR as PASSED [1].              | Marks original PR as FAILED.          |
